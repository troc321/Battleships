 ...
 * Please type in your name[s] below if you change the code:
 * Esther Brandle, May 2021
 *
 */
...
int main() {
    ...
    // Initialize various win statistics here
    ...
    // This only should happen once per program run.
    srand(time(NULL));
...
PlayerV2* getPlayer( int playerId, int boardSize ) {
    switch( playerId ) {
        // Professor provided
        default:
        case 0: return new BlackPearl( boardSize );
        case 1: return new BattlePearl( boardSize );
        case 2: return new GamblerPlayerV2( boardSize );
        case 3: return new CleanPlayerV2( boardSize );
    }
}
